<?php
  ...
  $appl = "SELECT * FROM applications WHERE reg=\"$_SESSION[reg]\"";
  $res = mysqli_query($con, $appl);
 ?>
    ...
    <div class="container-fluid">
      ...
      <div class="card mb-3">
        <div class="card-header">
          <i class="fa fa-table"></i> Registered Jobs</div>
        <div class="card-body">
            ...
            <table class="table" id="dataTable" width="100%" cellspacing="0">
              ...
              <tbody>
                <?php while ($row = mysqli_fetch_assoc($res)) {
                  $sql = "SELECT * FROM jobs WHERE jid=$row[jid]";
                  $jbs = mysqli_fetch_assoc(mysqli_query($con, $sql));
                  $cmp = "SELECT * FROM rec WHERE ema='$jbs[con]'";
                  $com = mysqli_fetch_assoc(mysqli_query($con, $cmp));
                  if ($jbs['status']==='1') {?>
                  <tr>
                  <td> <?php echo $com['com']; ?></td>
                  <td><?php echo $jbs['pos']; ?></td>
                  <td><?php echo $jbs['com']; ?></td>
                  <td><?php echo $jbs['rd']; ?></td>
                  <td><?php
                  if($row['res']==='1') { echo "<font color=green>Selected</font>";}
                  elseif($row['res']==='2') { echo "<font color=red>Not Selected</font>"; }
                  else{ echo "NA"; }
                  ?></td>
                  <td>
                    <form action="details.php" method="post">
                      <input type="text" name="reg" value="<?php echo $_SESSION['reg']; ?>" hidden>
                      <button class="btn btn-secondary" type="submit" name="results" value="<?php echo $row['jid']; ?>"><i class="fa fa-info-circle"></i>
                      </button></form>
                  </td></tr>
                <?php }} ?>
              </tbody>
